...
function valueSubcategory(category, sub) {
  const files=sub.attachments || [], editable=can('valores'), inputId=`sub-upload-${category.id}-${sub.id}`;
  return `<section class="value-subcategory-detail"><button class="back-subcategory" data-action="back-subcategory" data-id="${esc(category.id)}">← Voltar para ${esc(category.name)}</button><div class="value-subcategory-title"><span>${esc(sub.icon)}</span><div><h3>${esc(sub.name)}</h3><p>Subcategoria de ${esc(category.name)}</p></div></div><label>Observações<textarea data-value-notes="${esc(category.id)}:${esc(sub.id)}" maxlength="10000" ${editable?'':'readonly'} placeholder="Escreva as observações desta subcategoria…">${esc(sub.observations || '')}</textarea></label>${editable?`<button class="button primary" data-action="save-subcategory" data-id="${esc(category.id)}" data-subcategory="${esc(sub.id)}">Salvar observações</button>`:''}<section class="category-attachments"><div class="attachments-heading"><h3>Anexos</h3>${editable?`<label class="button" for="${esc(inputId)}">＋ Adicionar anexo</label><input class="attachment-input" id="${esc(inputId)}" data-sub-upload="${esc(category.id)}:${esc(sub.id)}" type="file" accept=".png,.jpg,.jpeg,.webp,.pdf,.txt" multiple>`:''}</div><p class="field-hint">PNG, JPG, WebP, PDF ou TXT · até 2 MB por arquivo · 5 anexos por subcategoria</p><div class="attachment-list">${files.length?files.map(file=>`<div class="attachment-row">${file.type.startsWith('image/')?`<img class="attachment-preview" src="${esc(file.data)}" alt="Prévia de ${esc(file.name)}">`:`<span>${icon('pedidos')}</span>`}<div><strong>${esc(file.name)}</strong><small>${(file.size/1024).toFixed(1)} KB</small></div><button class="button" data-action="preview-sub-attachment" data-id="${esc(category.id)}" data-subcategory="${esc(sub.id)}" data-file="${esc(file.id)}">Pré-visualizar</button>${editable?`<button class="icon-button" data-action="remove-sub-attachment" data-id="${esc(category.id)}" data-subcategory="${esc(sub.id)}" data-file="${esc(file.id)}" aria-label="Remover ${esc(file.name)}">×</button>`:''}</div>`).join(''):'<p class="muted">Nenhum anexo nesta subcategoria.</p>'}</div></section></section>`;
}
...
async function uploadSubcategoryAttachments(input) {
  const [categoryId,subId]=input.dataset.subUpload.split(':');const category=data.valores.find(row=>row.id===categoryId),sub=category?.subcategories?.find(row=>row.id===subId);if(!sub||!input.files.length)return;
  input.disabled=true;
  try {const files=await Promise.all([...input.files].map(async file=>{const type=file.type || (file.name.toLowerCase().endsWith('.txt')?'text/plain':'');if(file.size>MAX_FILE_BYTES)throw new Error('Cada arquivo deve ter no máximo 2 MB.');if(!ATTACHMENT_TYPES.includes(type))throw new Error('Use arquivos PNG, JPG, WebP, PDF ou TXT.');const encoded=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(new Error('Não foi possível ler o arquivo.'));reader.readAsDataURL(new Blob([file],{type}));});return {id:crypto.randomUUID(),name:file.name,type,size:file.size,data:encoded};}));sub.attachments=validateAttachments([...(sub.attachments||[]),...files]);await save('valores',{...category,color:category.color || '#7c8cff',subcategories:category.subcategories},category.id);toast('Anexo adicionado à subcategoria.');}catch(error){toast(error.name==='QuotaExceededError'?'O armazenamento da demonstração está cheio. Remova anexos ou use arquivos menores.':error.message);}finally{input.disabled=false;input.value='';}
}
...
$('#pageContent').addEventListener('change',event=>{if(event.target.dataset.upload)uploadAttachments(event.target);if(event.target.dataset.subUpload)uploadSubcategoryAttachments(event.target);});
...
$('#pageContent').addEventListener('click',async event=>{
  const button=event.target.closest('button');if(!button||!['save-notes','preview-attachment','download-attachment','remove-attachment'].includes(button.dataset.action))return;
  const row=data.investigativa.find(item=>item.id===button.dataset.id);if(!row)return;
  try{
    if(button.dataset.action==='preview-attachment'){const file=(row.attachments||[]).find(item=>item.id===button.dataset.file);if(!file)throw new Error('Anexo não encontrado.');window.open(file.data,'_blank','noopener');return;}
