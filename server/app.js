...
export function createApp({ database=':memory:', frontendUrl, apiUrl, clientId='', clientSecret='', adminDiscordId='', fetchImpl=fetch }) {
  ...
  async function body(req){const chunks=[];let size=0;const limit=req.url.startsWith('/api/investigativa')?9*1024*1024:65536;for await(const chunk of req){size+=chunk.length;if(size>limit)fail(413,'Solicitação muito grande.');chunks.push(chunk);}try{return size?JSON.parse(Buffer.concat(chunks).toString('utf8')):{};}catch{fail(400,'JSON inválido.');}}
